...
The server intentionally does not implement Git's legacy dumb HTTP static-object interface. Discovery requests without a `service=` query are rejected with a specific bad-request response; this is treated as expected smart-HTTP-only behavior rather than a compatibility bug.
