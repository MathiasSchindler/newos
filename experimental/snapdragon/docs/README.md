...
The decoder's stateless MLP blocks execute as one cached QNN graph per layer. CPU LayerNorm and residual addition retain FP32 state, while each graph receives and returns one FP16 width-sized vector. Cross-attention uses the same per-layer graph model but binds the shared K/V memory handles produced once per window. On the five-minute Small reference, the combined path removed CPU feed-forward and cross-attention, reduced process CPU time from 225.344 to 69.094 seconds, and raised exact host-observed NPU duty from 16.41% to 36.37% versus the retry-policy baseline. Detailed commands, quality notes, memory tradeoffs, and current priorities are tracked in [benchmark.md](benchmark.md).
