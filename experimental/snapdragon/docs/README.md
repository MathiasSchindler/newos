...
The decoder's stateless MLP blocks now execute as one cached QNN graph per layer. CPU LayerNorm and residual addition retain FP32 state, while each graph receives and returns one FP16 width-sized vector. On the five-minute Small reference, this raised exact host-observed NPU duty from 1.24% to 15.28%, reduced process CPU time by 24.8%, and improved throughput from 1.714x to 2.590x real time, with a best observed 2.684x run. Detailed commands, quality notes, memory tradeoffs, and current priorities are tracked in [benchmark.md](benchmark.md).
