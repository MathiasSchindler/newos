...
The narrow ABI in `../src/shared/qnn_abi.h` was checked against exact QAIRT `2.42.0.251225` QNN core 2.32 headers acquired from Qualcomm's Community SDK archive. Compiler-derived ARM64 sizes and offsets are enforced with static assertions; no offsets are inferred from binaries and no proprietary SDK headers are copied into the repository. The validated provider on this machine is `HTP_QTI_AISW`, backend ID 6, from QAIRT `2.42.0.251225135753_193295`, with QNN core API 2.32.0 and backend API 5.41.0. See [npu-plan.md](npu-plan.md) for the incremental path to a Whisper-like model.
