...
`tools/shared/test-translate-gui.py` against the actual native controls and NPU. Python
...
splitting and bounds. `tools/translate/test-translate.py --documents` tests the real NPU
...
`./experimental/snapdragon/tools/translate/build-gemma.ps1 -Translate`. The VS Code task
...
`experimental/snapdragon/build/calibration-venv/Scripts/python.exe -B experimental/snapdragon/tools/translate/test-translate.py --hardware`.
...
4. Do not split `npu_probe.c`, rename symbols, change artifact formats, or extract
   ...
   to attribute.
5. Reserve `src/apps/translate` as the owner of all TranslateGemma-specific runtime
...
- The capability suite is builder-only and runs with
   `tools/translate/test-gemma-stage1.ps1`; the production `npu_probe.exe` neither links the
...
Materialize `src/apps/translate` with a canonical descriptor for TranslateGemma 4B.
...
- `src/apps/translate/gemma_model.h/.c` pins the 34-layer, 2560-wide text descriptor,
   the 5-local/1-global schedule, and the immutable source revision.
- `tools/translate/translategemma-models.json` records all 15 official files with complete
...
- `tools/translate/fetch-translategemma.ps1` requires `-AcceptGemmaLicense`, reads a token
...
- `tools/translate/validate-translategemma.py` rejects catalog, file, configuration,
...
- `tools/translate/test-gemma-stage2.ps1` runs the descriptor and acquisition gates;
...
- `src/apps/translate/gemma_artifact.h/.c` defines a 256-byte little-endian v1
...
- `tools/translate/export-translategemma.py` revalidates the pinned Stage 2 checkpoint,
...
- `tools/translate/test-gemma-stage3.ps1` runs the C reader/corruption/unpack checks and
   Python streaming quantization tests. `tools/translate/export-translategemma.py
...
- `src/apps/translate/gemma_tokenizer.h/.c` provides an allocation-free table reader,
...
- The existing `tools/translate/export-translategemma.py --tokenizer-only` path exports
...
- `tools/translate/build-gemma.ps1 -Test` builds and executes the freestanding ARM64 C
...
- `tools/translate/translategemma-reference.py` is an offline reference module invoked by
...
- `src/apps/translate/gemma_numeric.h/.c` supplies only small allocation-free scalar
...
The tool-private `src/apps/translate/gemma_block.h/.c` builds a fixed three-token
...
`tools/translate/translategemma-quality.json` is a versioned pilot corpus: 24 diagnostic
...
$quality = 'experimental/snapdragon/tools/translate/translategemma-quality.py'
& $python -m pip install -r experimental/snapdragon/tools/shared/calibration-requirements.txt
...
- [Existing Snapdragon QNN findings](../platform/snapdragon-x-qnn.md)
- [Existing Whisper benchmark record](whisper-benchmark.md)