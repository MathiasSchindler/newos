...
- Generation: the current prompt fixes German transcription without timestamps. Decoding suppresses the checkpoint's non-speech token list and timestamp range and stops at EOT or the configured token limit. Tiny remains greedy. Base retries a bounded deterministic temperature schedule only when repeated token bigrams and trigrams reject the greedy result, then emits the accepted buffered UTF-8 sequence.
...
- Per-token QNN offload decision: retain CPU self/cross-attention until a verified persistent device-memory path can keep mutable KV state resident. Stateless MLP offload is different: each layer transfers only one width-sized FP16 input and output while its two matrices remain static in QNN. The validated Small path uses 12 cached MLP graphs, removes CPU feed-forward work, raises exact host-observed NPU duty from 1.24% to 15.28%, and reduces the five-minute wall time by 33.8% (36.2% in the best observed run).
...
- Small model: the pinned multilingual checkpoint uses width 768, FFN width 3072, 12 heads, and 12 encoder/decoder layers. Its decoder is 307,164,768 bytes, cross-K/V bundle is 28,333,152 bytes, frontend is 6,214,752 bytes, encoder is 170,109,024 bytes, and serialized QNN context payload is 210,792,448 bytes.
...
- Bottleneck and offload decision: Small spent 7.236 seconds in CPU cross-attention and 5.461 seconds in feed-forward work. QNN encoder compilation also reported 356,886,528 spill bytes and 385,198,080 fill bytes. Additional decoder offload remains unattractive because the available restored-graph contract does not provide persistent mutable device KV state; per-token uploads and dispatches would replace local bandwidth with larger transfer and synchronization costs.