...
static u32 prefill_norm(OcrAttentionGraph *builder, u32 input, u8 **constants, u32 *axis) {
    u32 width[1] = {1536}, shape[2] = {64,1536};
...
static u32 prefill_rope(OcrAttentionGraph *builder, u32 input, u32 heads, u32 cosine, u32 sine, u32 rotation, u32 signs, int tap) {
    u32 shape[3] = {64,heads,128};
...
static int prefill_layer(const QnnInterfaceV2 *api, QnnContextHandle context, u32 index) {
    ...
    u32 flat[2] = {64,1536}, head_shape[3] = {64,16,128}, batch[3] = {16,64,128}, keys[3] = {16,128,64};
    u32 scores_shape[3] = {16,64,64}, frequency_shape[3] = {64,1,128}, one_shape[1] = {1}, head_width[1] = {128}, group_width[1] = {16};
    ...
    u32 norm0 = prefill_norm(&builder,input,&constants,&scalar_axis);
    u32 query_raw = vision_linear(&builder,norm0,64,1536,2048,&constants,0,0);
    u32 key_raw = vision_linear(&builder,norm0,64,1536,1024,&constants,0,0);
    u32 value_raw = vision_linear(&builder,norm0,64,1536,1024,&constants,0,1);
    ...
    u32 query = prefill_rope(&builder,query_raw,16,cosine,sine,rotation,sign,0);
    u32 key = prefill_rope(&builder,key_raw,8,cosine,sine,rotation,sign,1);
    u32 kv_shape[3] = {64,8,128};
    ...
    u32 ids[2] = {key,group}; QnnParam group_axis = attention_axis("axis",1);
    u32 expanded_key = attention_op(&builder,"Gather",ids,2,3,head_shape,QNN_DATATYPE_FLOAT_16,0,&group_axis,1);
    ids[0] = value;
    u32 expanded_value = attention_op(&builder,"Gather",ids,2,3,head_shape,QNN_DATATYPE_FLOAT_16,0,&group_axis,1);
    u32 query_batch = attention_transpose(&builder,query,batch,permutation);
    u32 key_batch = attention_transpose(&builder,expanded_key,batch,permutation);
    u32 key_transposed = attention_transpose(&builder,key_batch,keys,key_permutation);
    u32 value_batch = attention_transpose(&builder,expanded_value,batch,permutation);
    ...
    u32 mask_shape[3] = {1,64,64}; ids[0] = scores;
    ...
    u32 reduced[2] = {16,64}, broadcast[3] = {16,64,1};
    ...
    u32 ordered = attention_transpose(&builder,attended,head_shape,permutation), context_shape[2] = {64,2048};
    u32 flattened = attention_op(&builder,"Reshape",&ordered,1,2,context_shape,QNN_DATATYPE_FLOAT_16,0,0,0);
    u32 projected = vision_linear(&builder,flattened,64,2048,1536,&constants,0,0);
    u32 norm1 = prefill_norm(&builder,projected,&constants,&scalar_axis);
    ids[0] = input; ids[1] = norm1;
    u32 residual = attention_op(&builder,"ElementWiseAdd",ids,2,2,flat,QNN_DATATYPE_FLOAT_16,0,0,0);
    u32 norm2 = prefill_norm(&builder,residual,&constants,&scalar_axis);
    u32 gate_up = vision_linear(&builder,norm2,64,1536,9216,&constants,0,0);
    u32 wide[2] = {64,4608}, wide_width[1] = {4608}, branches[2]; QnnParam select_axis = attention_axis("axis",1);
    ...
    u32 down = vision_linear(&builder,gated,64,4608,1536,&constants,0,0);
    u32 norm3 = prefill_norm(&builder,down,&constants,&scalar_axis);
    ...
    if (constants != fixture_data+61354148 || !vision_execution(&builder,input,prefill_input.embeddings,64*1536,outputs,8,prefill_output)) return 0;
    for (u32 row = 0; row < 16*64; ++row) {
        float total = 0;
        for (u32 column = 0; column < 64; ++column) {
            union {u16 bits; _Float16 value;} probability; probability.bits = prefill_output[622592+row*64+column];
            if (!(probability.value >= 0 && probability.value <= 1) ||
                ((column > row%64 || column >= prefill_input.count) && probability.value != 0)) return 0;
            total += (float)probability.value;
        }
        if (!(total >= 0.997f && total <= 1.003f)) return 0;
    }
    if (!capture_tensor(index,".text-input.f16",prefill_input.embeddings,64*1536*2) ||
        !capture_tensor(index,".text-taps.f16",prefill_output,sizeof(prefill_output))) return 0;
    for (u32 offset = 0; offset < 64*1536; ++offset) prefill_input.embeddings[offset] = prefill_output[4*64*1536+offset];
...
static int prefill_forward(const QnnInterfaceV2 *api, QnnBackendHandle backend, QnnDeviceHandle device,
    ...
    for (u32 index = 0; index <= 16; ++index) {
        ...
        *context = 0;
        ...
        if (index < 16) { if (!prefill_layer(api,*context,index)) return 0; }
        else {
            OcrAttentionGraph builder = {0}; builder.api = api; builder.good = 1;
            ...
            u8 *gamma = prefill_shared+160; u32 normalized = prefill_norm(&builder,input,&gamma,&axis);
