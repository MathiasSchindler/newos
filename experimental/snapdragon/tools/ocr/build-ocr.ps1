...
try {
    ...
    $sources = @('experimental/snapdragon/src/apps/ocr/ocr_model.c', 'src/shared/crypto/sha256.c')
    if ($TestTokenizer -or $TestImages) {
        $binary = Join-Path $BuildDir 'ocr-tokenizer-test.exe'
        $flags += '-DOCR_TOKENIZER_TEST'
        $sources += @('experimental/snapdragon/src/apps/ocr/ocr_tokenizer.c', 'experimental/snapdragon/src/apps/ocr/ocr_tokenizer_test.c')
    }
    if ($TestImages -or $PrepareImages -or $Vision) {
        ...
        $sources += @('experimental/snapdragon/src/apps/ocr/ocr_image.c', 'experimental/snapdragon/src/apps/ocr/ocr_image_file.c')
        $sources += @('experimental/snapdragon/src/apps/ocr/ocr_png.c', 'src/shared/compression/zlib.c')
        if ($TestImages -or $Prefill) { $sources += 'experimental/snapdragon/src/apps/ocr/ocr_text.c' }
        if ($TestImages) {
    ...
    if ($TestHtp -or $Vision) {
        ...
        $sources += 'experimental/snapdragon/src/apps/ocr/ocr_htp.c'
        if (-not ($TestTokenizer -or $TestImages)) { $sources += 'experimental/snapdragon/src/apps/ocr/ocr_tokenizer.c' }
    }
    if ($Gui) {
        ...
        $sources = @('experimental/snapdragon/src/apps/ocr/ocr_gui.c','experimental/snapdragon/src/apps/ocr/ocr_image.c','experimental/snapdragon/src/apps/ocr/ocr_png.c','src/shared/compression/zlib.c')
    }
