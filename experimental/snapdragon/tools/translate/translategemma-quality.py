...
def candidate_payload(config):
    manifest = json.loads((TOOLS.parent.parent / "models/translategemma-4b-stage3/manifest.json").read_text())
    total = 0
...
def prepare(corpus_path, split, identifiers, maximum):
    ...
    model = TOOLS.parent.parent / "data/translategemma-4b"
    ...
    return {"schema_version": 1, "split": split, "corpus_sha256": digest(corpus_path),
            ...
            "weight_manifest_sha256": digest(TOOLS.parent.parent / "models/translategemma-4b-stage3/manifest.json"),
