...
MACOS_FREESTANDING_GIT_SOURCES := $(TLS_SOURCES) $(CRYPTO_SOURCES) src/shared/compression/zlib.c src/platform/macos/tls.c
...
$(BUILD_DIR)/git: src/tools/git.c $(TLS_SOURCES) $(CRYPTO_SOURCES) $(SHARED_SOURCES) src/shared/runtime.h src/shared/platform.h src/shared/tool_util.h src/shared/crypto/sha1.h src/shared/compression/zlib.h $(HOST_TLS_PLATFORM_SOURCE) $(HOST_PLATFORM_SOURCES) $(SELFHOST_CC_DEP) | $(BUILD_DIR)
	mkdir -p $(dir $@) && $(CC) $(HOST_CFLAGS) $< $(TLS_SOURCES) $(CRYPTO_SOURCES) $(SHARED_SOURCES) $(HOST_TLS_PLATFORM_SOURCE) $(HOST_PLATFORM_SOURCES) -o $@

$(TARGET_BUILD_DIR)/git: src/tools/git.c $(TARGET_REUSABLE_OBJECTS) $(FREESTANDING_TLS_OBJECTS) $(FREESTANDING_CRYPTO_OBJECTS) $(FREESTANDING_TARGET_TLS_PLATFORM_OBJECT) src/shared/runtime.h src/shared/platform.h src/shared/tool_util.h src/shared/crypto/sha1.h src/shared/compression/zlib.h $(TARGET_CRT) $(TARGET_ARCH_DIR)/syscall.h src/platform/linux/common.h | $(TARGET_BUILD_DIR)
