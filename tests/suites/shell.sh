...
printf 'export BAD-NAME=value\n' | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/export_invalid.out" 2>&1 || invalid_export_status=$?
...
printf 'echo first\necho second\nhistory\n' | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/history.out"
...
printf '%s\n' "$long_pipeline" | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/long_pipeline.out"
...
printf '%s\n' "$many_args" | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/many_args.out"
...

{
    ...
    printf 'history\n'
} | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/long_history.out"
...

{
    ...
    printf 'jobs\n'
} | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/many_jobs.out"
...

{
    ...
    printf 'f40\n'
} | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/many_aliases_functions.out"
...
printf 'alias say="echo two words"\nsay\nshow() { echo "$1"; }\nshow "quoted text"\n' | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/stability.out"
...
echo "shift:$1|$#"
EOF
"${TEST_BIN_DIR}/sh" "$WORK_DIR/args.sh" alpha "beta gamma" > "$WORK_DIR/args.out"
...
printf 'cat <<EOF\ncleanup-check\nEOF\n' | "${TEST_BIN_DIR}/sh" > "$WORK_DIR/heredoc_cleanup.out"
