...
assert_command_succeeds "${TEST_BIN_DIR}/touch" -t 200102030405.06 "$WORK_DIR/tree/root.txt"

"${TEST_BIN_DIR}/ls" -S "$WORK_DIR/sortdir" > "$WORK_DIR/size.out"
...
"${TEST_BIN_DIR}/ls" -R "$WORK_DIR/tree" > "$WORK_DIR/recursive.out"
assert_file_contains "$WORK_DIR/recursive.out" 'subdir:' "ls -R did not list the nested directory"

"${TEST_BIN_DIR}/ls" -1F "$WORK_DIR/tree" > "$WORK_DIR/classify.out"
...
"${TEST_BIN_DIR}/ls" -A "$WORK_DIR/tree" > "$WORK_DIR/almost_all.out"
...
"${TEST_BIN_DIR}/ls" -p "$WORK_DIR/tree" > "$WORK_DIR/dirsuffix.out"
...
"${TEST_BIN_DIR}/ls" -r "$WORK_DIR/reverse" > "$WORK_DIR/reverse.out"
...
"${TEST_BIN_DIR}/ls" -isF "$WORK_DIR/tree" > "$WORK_DIR/inodes.out"
...
"${TEST_BIN_DIR}/ls" -l "$WORK_DIR/tree" > "$WORK_DIR/long.out"
...
"${TEST_BIN_DIR}/ls" -ln "$WORK_DIR/tree/root.txt" > "$WORK_DIR/numeric.out"
...
"${TEST_BIN_DIR}/ls" --color=always "$WORK_DIR/tree" > "$WORK_DIR/color.out"
if ! LC_ALL=C grep -q "$(printf '\033')\\[" "$WORK_DIR/color.out"; then
    fail "ls --color=always did not emit ANSI color sequences"
fi

NO_COLOR=1 "${TEST_BIN_DIR}/ls" --color=auto "$WORK_DIR/tree" > "$WORK_DIR/no_color.out"
