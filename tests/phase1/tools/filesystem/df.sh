...
"${TEST_BIN_DIR}/df" > "$WORK_DIR/basic.out"
assert_file_contains "$WORK_DIR/basic.out" '^Filesystem[[:space:]]' "df header missing"

"${TEST_BIN_DIR}/df" -h "$WORK_DIR" > "$WORK_DIR/human.out"
...
"${TEST_BIN_DIR}/df" -k "$WORK_DIR" > "$WORK_DIR/blocks.out"
assert_file_contains "$WORK_DIR/blocks.out" '1K-blocks' "df -k did not label 1K-sized output columns"

"${TEST_BIN_DIR}/df" -iT "$WORK_DIR" > "$WORK_DIR/inodes.out"
