...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -cf sample.tar tar_src
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -tf sample.tar > tar_list.out
)
...
(
    ...
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -xf "$WORK_DIR/sample.tar"
)
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/tar" --exclude='tar_filter_src/skip/*' -cf filtered.tar tar_filter_src
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -tf filtered.tar 'tar_filter_src/keep/*' > filtered_list.out
)
...
(
    ...
    assert_command_succeeds "${TEST_BIN_DIR}/tar" --strip-components=1 -xf "$WORK_DIR/filtered.tar" 'tar_filter_src/keep/*'
)
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -cf advanced.tar tar_advanced_src
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -tf advanced.tar > advanced_list.out
)
...
(
    ...
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -xf "$WORK_DIR/advanced.tar"
)
...
if [ "$("${TEST_BIN_DIR}/readlink" "$WORK_DIR/tar_advanced_extract/tar_advanced_src/links/link.txt")" != 'target.txt' ]; then
...
if command -v tar >/dev/null 2>&1; then
    ...
    (
        ...
        assert_command_succeeds "${TEST_BIN_DIR}/tar" -tf external.tar > external_list.out
    )
    ...
    (
        ...
        assert_command_succeeds "${TEST_BIN_DIR}/tar" -xf "$WORK_DIR/external.tar"
    )
    ...
    if [ "$("${TEST_BIN_DIR}/readlink" "$WORK_DIR/external_tar_extract/external_tar_src/link.txt")" != 'dir/target.txt' ]; then
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/tar" -cf symlink_escape.tar tar_escape_src
)
ln -s "$WORK_DIR/tar_escape_outside" "$WORK_DIR/tar_escape_extract/tar_escape_src"
tar_escape_status=0
(
    cd "$WORK_DIR/tar_escape_extract"
    "${TEST_BIN_DIR}/tar" -xf "$WORK_DIR/symlink_escape.tar"
...
printf 'one\n' > "$WORK_DIR/a.txt"
printf 'two\n' > "$WORK_DIR/b.txt"
assert_command_succeeds "${TEST_BIN_DIR}/ar" rc "$WORK_DIR/test.a" "$WORK_DIR/a.txt" "$WORK_DIR/b.txt"
assert_command_succeeds "${TEST_BIN_DIR}/ar" t "$WORK_DIR/test.a" > "$WORK_DIR/ar_list.out"
...
(
    ...
    assert_command_succeeds "${TEST_BIN_DIR}/ar" x "$WORK_DIR/test.a"
)
...
(
    ...
    "${TEST_BIN_DIR}/ar" x "$WORK_DIR/ar_escape/evil.a"
