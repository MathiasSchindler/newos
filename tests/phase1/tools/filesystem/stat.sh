...
"${TEST_BIN_DIR}/stat" "$WORK_DIR/data.txt" > "$WORK_DIR/basic.out"
...
stat_format_out=$("${TEST_BIN_DIR}/stat" -c '%F %a %n' "$WORK_DIR/data.txt" | tr -d '\r')
...
stat_follow_out=$("${TEST_BIN_DIR}/stat" -L -c '%F' "$WORK_DIR/data.link" | tr -d '\r\n')
assert_text_equals "$stat_follow_out" 'file' "stat -L did not follow the symlink"

"${TEST_BIN_DIR}/stat" -f "$WORK_DIR" > "$WORK_DIR/filesystem.out"
