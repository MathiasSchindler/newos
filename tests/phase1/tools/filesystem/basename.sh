...
actual_basename=$("${TEST_BIN_DIR}/basename" "$WORK_DIR/path/example.txt" .txt | tr -d '\r\n')
assert_text_equals "$actual_basename" 'example' "basename did not strip the suffix from a single path"

printf 'a\n' > "$WORK_DIR/alpha.txt"
printf 'b\n' > "$WORK_DIR/beta.txt"
basename_multi=$("${TEST_BIN_DIR}/basename" -a -s .txt "$WORK_DIR/alpha.txt" "$WORK_DIR/beta.txt" | tr -d '\r')
...
"${TEST_BIN_DIR}/basename" --multiple --suffix=.txt --zero "$WORK_DIR/alpha.txt" "$WORK_DIR/beta.txt" | tr '\0' '\n' > "$WORK_DIR/zero.out"
