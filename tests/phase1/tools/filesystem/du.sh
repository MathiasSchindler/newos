...
"${TEST_BIN_DIR}/du" "$WORK_DIR/tree" > "$WORK_DIR/basic.out"
assert_file_contains "$WORK_DIR/basic.out" 'tree$' "du did not report the directory total"

du_h_out=$("${TEST_BIN_DIR}/du" -sh "$WORK_DIR/tree" | tr -d '\r')
