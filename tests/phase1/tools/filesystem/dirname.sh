...
actual_dirname=$("${TEST_BIN_DIR}/dirname" "$WORK_DIR/sub/inner/example.txt" | tr -d '\r\n')
assert_text_equals "$actual_dirname" "$WORK_DIR/sub/inner" "dirname returned the wrong parent path"

dirname_multi=$("${TEST_BIN_DIR}/dirname" "$WORK_DIR/sub/inner/example.txt" "$WORK_DIR/root.txt" | tr -d '\r')
...
"${TEST_BIN_DIR}/dirname" --zero "$WORK_DIR/sub/inner/example.txt" "$WORK_DIR/root.txt" | tr '\0' '\n' > "$WORK_DIR/zero.out"
