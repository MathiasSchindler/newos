...
assert_command_succeeds "${TEST_BIN_DIR}/cp" -r "$WORK_DIR/src" "$WORK_DIR/tree_copy"
assert_file_contains "$WORK_DIR/tree_copy/sub/file.txt" '^beta$' "cp -r did not preserve nested file contents"

assert_command_succeeds "${TEST_BIN_DIR}/touch" -t 200102030405.06 "$WORK_DIR/src/root.txt"
assert_command_succeeds "${TEST_BIN_DIR}/cp" -p "$WORK_DIR/src/root.txt" "$WORK_DIR/preserved.txt"
cp_src_mtime=$("${TEST_BIN_DIR}/stat" -c '%Y' "$WORK_DIR/src/root.txt" | tr -d '\r\n')
cp_dest_mtime=$("${TEST_BIN_DIR}/stat" -c '%Y' "$WORK_DIR/preserved.txt" | tr -d '\r\n')
assert_text_equals "$cp_dest_mtime" "$cp_src_mtime" "cp -p did not preserve the modification time"

printf 'A\n' > "$WORK_DIR/src/a.txt"
printf 'B\n' > "$WORK_DIR/src/b.txt"
assert_command_succeeds "${TEST_BIN_DIR}/cp" "$WORK_DIR/src/a.txt" "$WORK_DIR/src/b.txt" "$WORK_DIR/multi"
...
assert_command_succeeds "${TEST_BIN_DIR}/cp" -n "$WORK_DIR/replace.txt" "$WORK_DIR/keep.txt"
keep_text=$(tr -d '\r\n' < "$WORK_DIR/keep.txt")
assert_text_equals "$keep_text" 'keep' "cp -n overwrote an existing destination"

printf 'older-source\n' > "$WORK_DIR/update_src.txt"
assert_command_succeeds "${TEST_BIN_DIR}/touch" -d @1000000000 "$WORK_DIR/update_src.txt"
printf 'newer-destination\n' > "$WORK_DIR/update_dest.txt"
assert_command_succeeds "${TEST_BIN_DIR}/touch" -d @2000000000 "$WORK_DIR/update_dest.txt"
assert_command_succeeds "${TEST_BIN_DIR}/cp" -u "$WORK_DIR/update_src.txt" "$WORK_DIR/update_dest.txt"
update_text=$(tr -d '\r\n' < "$WORK_DIR/update_dest.txt")
...
assert_command_succeeds "${TEST_BIN_DIR}/cp" -a "$WORK_DIR/archive-link" "$WORK_DIR/archive-dest"
[ -L "$WORK_DIR/archive-dest/archive-link" ] || fail "cp -a did not preserve the symlink itself"
archive_link_out=$("${TEST_BIN_DIR}/readlink" "$WORK_DIR/archive-dest/archive-link" | tr -d '\r\n')
...
assert_command_succeeds "${TEST_BIN_DIR}/cp" -r "$DEEP_SRC" "$WORK_DIR/deep_copy"
...
printf 'odd-data\n' > "$ODD_FILE"
assert_command_succeeds "${TEST_BIN_DIR}/cp" "$ODD_FILE" "$WORK_DIR/odd-copy.txt"
...
self_copy_status=0
"${TEST_BIN_DIR}/cp" "$WORK_DIR/self-link.txt" "$WORK_DIR/self-target.txt" >/dev/null 2>&1 || self_copy_status=$?
