...
"${TEST_BIN_DIR}/head" -n 2 "$WORK_DIR/input.txt" > "$WORK_DIR/head.out"
...
"${TEST_BIN_DIR}/tail" -n 2 "$WORK_DIR/input.txt" > "$WORK_DIR/tail.out"
...
head_bytes=$("${TEST_BIN_DIR}/head" -c 5 "$WORK_DIR/bytes.txt" | tr -d '\r\n')
assert_text_equals "$head_bytes" 'abcde' "head -c returned the wrong leading bytes"
tail_bytes=$("${TEST_BIN_DIR}/tail" -c 5 "$WORK_DIR/bytes.txt" | tr -d '\r\n')
assert_text_equals "$tail_bytes" '12345' "tail -c returned the wrong trailing bytes"

"${TEST_BIN_DIR}/head" -n +3 "$WORK_DIR/input.txt" > "$WORK_DIR/head_plus.out"
...
"${TEST_BIN_DIR}/tail" -v "$WORK_DIR/input.txt" > "$WORK_DIR/tail_v.out"
