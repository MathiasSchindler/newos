...
printf 'alpha\n\n\nbeta\n' > "$WORK_DIR/flags.txt"
"${TEST_BIN_DIR}/cat" -b "$WORK_DIR/flags.txt" > "$WORK_DIR/cat_b.out"
assert_file_contains "$WORK_DIR/cat_b.out" '^[[:space:]]*2[[:space:]]beta$' "cat -b did not skip blank lines when numbering"
"${TEST_BIN_DIR}/cat" -s "$WORK_DIR/flags.txt" > "$WORK_DIR/cat_s.out"
...
"${TEST_BIN_DIR}/cat" -A "$WORK_DIR/cat_visible.txt" > "$WORK_DIR/cat_visible.out"
...
printf 'odd-data\n' > "$ODD_FILE"
"${TEST_BIN_DIR}/cat" "$ODD_FILE" > "$WORK_DIR/odd.out"
