...
"${TEST_BIN_DIR}/seq" 2 2 6 > "$WORK_DIR/seq.out"
...
seq_decimal_out=$("${TEST_BIN_DIR}/seq" -w 0.5 0.25 1.0 | tr -d '\r')
...
seq_format_out=$("${TEST_BIN_DIR}/seq" -s ',' -f 'item%.1f' 1 0.5 2 | tr -d '\r\n')
assert_text_equals "$seq_format_out" 'item1.0,item1.5,item2.0' "seq -f or separator handling failed"

seq_scientific_out=$("${TEST_BIN_DIR}/seq" 1e3 2e3 5e3 | tr -d '\r')
...
seq_big_out=$("${TEST_BIN_DIR}/seq" 999999999999999999999999999999 1 1000000000000000000000000000001 | tr -d '\r')
...
seq_exp_format_out=$("${TEST_BIN_DIR}/seq" -f '%.2e' 1000 1000 3000 | tr -d '\r')
...
seq_grouped_out=$("${TEST_BIN_DIR}/seq" -f "%'f" 1000 1000 3000 | tr -d '\r')
...
seq_signed_padded_out=$("${TEST_BIN_DIR}/seq" -s ',' -f '%+08.2f' 1 1 3 | tr -d '\r\n')
assert_text_equals "$seq_signed_padded_out" '+0001.00,+0002.00,+0003.00' "seq did not support signed zero-padded formatting"

"${TEST_BIN_DIR}/yes" ok | "${TEST_BIN_DIR}/head" -n 3 > "$WORK_DIR/yes.out"
