...
"${TEST_BIN_DIR}/strings" "$WORK_DIR/input.bin" > "$WORK_DIR/out.txt"
...
"${TEST_BIN_DIR}/strings" -a "$WORK_DIR/input.bin" > "$WORK_DIR/all.out"
assert_file_contains "$WORK_DIR/all.out" '^HELLO123$' "strings -a did not scan the full byte stream"

"${TEST_BIN_DIR}/strings" -t x "$WORK_DIR/input.bin" > "$WORK_DIR/offset.out"
...
"${TEST_BIN_DIR}/strings" -e l -n 4 "$WORK_DIR/wide_le.bin" > "$WORK_DIR/wide_le.out"
...
"${TEST_BIN_DIR}/strings" -d "$WORK_DIR/object_marker" > "$WORK_DIR/object_sections.out"
...
"${TEST_BIN_DIR}/strings" -d "$WORK_DIR/pe_strings.exe" > "$WORK_DIR/pe_sections.out"
