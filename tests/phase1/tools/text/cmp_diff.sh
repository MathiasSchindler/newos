...
assert_command_succeeds "${TEST_BIN_DIR}/cmp" -s "$WORK_DIR/left.txt" "$WORK_DIR/right.txt"

if "${TEST_BIN_DIR}/cmp" -s "$WORK_DIR/left.txt" "$WORK_DIR/other.txt"; then
    fail "cmp -s should report a difference for mismatched files"
fi

if "${TEST_BIN_DIR}/diff" -u "$WORK_DIR/left.txt" "$WORK_DIR/other.txt" > "$WORK_DIR/diff.out"; then
...
cmp_list_out=$("${TEST_BIN_DIR}/cmp" -l "$WORK_DIR/left.txt" "$WORK_DIR/other.txt" | tr -d '\r')
...
if "${TEST_BIN_DIR}/diff" -c "$WORK_DIR/left.txt" "$WORK_DIR/other.txt" > "$WORK_DIR/diff_c.out"; then
...
if "${TEST_BIN_DIR}/diff" --color=always -u "$WORK_DIR/left.txt" "$WORK_DIR/other.txt" > "$WORK_DIR/diff_color.out"; then
...
if "${TEST_BIN_DIR}/diff" --color=never -u "$WORK_DIR/left.txt" "$WORK_DIR/other.txt" > "$WORK_DIR/diff_plain.out"; then
...
if ! "${TEST_BIN_DIR}/diff" -wBi "$WORK_DIR/ws_left.txt" "$WORK_DIR/ws_right.txt" > "$WORK_DIR/diff_ignore.out"; then
