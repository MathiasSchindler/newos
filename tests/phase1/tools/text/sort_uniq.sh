...
"${TEST_BIN_DIR}/sort" "$WORK_DIR/input.txt" > "$WORK_DIR/sorted.out"
...
printf 'delta\nalpha\ncharlie\n' | "${TEST_BIN_DIR}/sort" > "$WORK_DIR/stdin_sorted.out"
...
"${TEST_BIN_DIR}/sort" -nru "$WORK_DIR/numeric.txt" > "$WORK_DIR/numeric.out"
...
"${TEST_BIN_DIR}/uniq" -c "$WORK_DIR/sorted.out" > "$WORK_DIR/uniq.out"
...
"${TEST_BIN_DIR}/uniq" -i -f 1 -w 5 "$WORK_DIR/uniq_controls.txt" > "$WORK_DIR/uniq_controls.out"
...
"${TEST_BIN_DIR}/sort" -n "$WORK_DIR/large_numbers.txt" > "$WORK_DIR/large_sorted.out"
...
"${TEST_BIN_DIR}/sort" -n "$WORK_DIR/huge_numbers.txt" > "$WORK_DIR/huge_sorted.out"
...
beta:1:first
EOF
"${TEST_BIN_DIR}/sort" -t : -k 2,2 "$WORK_DIR/key_fields.txt" > "$WORK_DIR/key_fields_lex.out"
...
"${TEST_BIN_DIR}/sort" -n -t : -k 2,2 "$WORK_DIR/key_fields.txt" > "$WORK_DIR/key_fields_numeric.out"
...
row-b    2
EOF
"${TEST_BIN_DIR}/sort" -k 2,2 "$WORK_DIR/blank_keys.txt" > "$WORK_DIR/blank_keys.out"
...
a10
EOF
"${TEST_BIN_DIR}/sort" -d "$WORK_DIR/dictionary.txt" > "$WORK_DIR/dictionary.out"
...
"${TEST_BIN_DIR}/sort" -i "$WORK_DIR/nonprinting.txt" > "$WORK_DIR/nonprinting.out"
...
Mar report
EOF
"${TEST_BIN_DIR}/sort" -M "$WORK_DIR/months.txt" > "$WORK_DIR/months.out"
...
file-1
EOF
"${TEST_BIN_DIR}/sort" -V "$WORK_DIR/versions.txt" > "$WORK_DIR/versions.out"
...
1.5K
EOF
"${TEST_BIN_DIR}/sort" --human-numeric-sort "$WORK_DIR/human_sizes.txt" > "$WORK_DIR/human_sizes.out"
...
pkg-c 1G
EOF
"${TEST_BIN_DIR}/sort" --human-numeric-sort -k 2,2 "$WORK_DIR/key_human.txt" > "$WORK_DIR/key_human.out"
...
if "${TEST_BIN_DIR}/sort" -c "$WORK_DIR/check_unsorted.txt" > /dev/null 2> "$WORK_DIR/check_unsorted.err"; then
...
if "${TEST_BIN_DIR}/sort" -C "$WORK_DIR/check_unsorted.txt" > "$WORK_DIR/check_quiet.out" 2> "$WORK_DIR/check_quiet.err"; then
    fail "sort -C accepted unsorted input"
fi
...
"${TEST_BIN_DIR}/sort" -c "$WORK_DIR/check_sorted.txt" > /dev/null 2> "$WORK_DIR/check_sorted.err" || \
    fail "sort -c rejected already sorted input"

printf 'alpha\nbeta\n' > "$WORK_DIR/merge_a.txt"
printf 'beta\ngamma\n' > "$WORK_DIR/merge_b.txt"
"${TEST_BIN_DIR}/sort" -m -u "$WORK_DIR/merge_a.txt" "$WORK_DIR/merge_b.txt" > "$WORK_DIR/merge.out"
...
"${TEST_BIN_DIR}/sort" -m $merge_args > "$WORK_DIR/merge_limit_ok.out"
...
"${TEST_BIN_DIR}/sort" -o "$WORK_DIR/output_in_place.txt" "$WORK_DIR/output_in_place.txt"
...
if "${TEST_BIN_DIR}/sort" -k 184467440737095516151 "$WORK_DIR/input.txt" > /dev/null 2> "$WORK_DIR/sort_key.err"
then
...
"${TEST_BIN_DIR}/sort" "$WORK_DIR/max_lines.txt" > "$WORK_DIR/max_lines.out"
...
"${TEST_BIN_DIR}/sort" -nu "$WORK_DIR/external_duplicates.txt" > "$WORK_DIR/external_duplicates.out"
...
"${TEST_BIN_DIR}/sort" -o "$WORK_DIR/external_in_place.txt" "$WORK_DIR/external_in_place.txt"
