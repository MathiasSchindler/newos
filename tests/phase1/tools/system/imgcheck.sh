...
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/valid.png")" "$WORK_DIR/valid.png${tab}png${tab}ok${tab}-${tab}valid PNG image" "imgcheck did not accept valid PNG"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --verbose "$WORK_DIR/valid.png")" "$WORK_DIR/valid.png: OK (png): valid PNG image" "imgcheck --verbose did not describe valid PNG"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample.jpg")" "$WORK_DIR/sample.jpg${tab}jpeg${tab}ok${tab}-${tab}valid JPEG image" "imgcheck did not accept valid JPEG"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/c2pa.jpg")" "$WORK_DIR/c2pa.jpg${tab}jpeg${tab}ok${tab}-${tab}valid JPEG image${tab}C2PA markers found; manifest store not recognized" "imgcheck did not report C2PA status"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample.gif")" "$WORK_DIR/sample.gif${tab}gif${tab}ok${tab}-${tab}valid GIF image" "imgcheck did not accept valid GIF"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample.bmp")" "$WORK_DIR/sample.bmp${tab}bmp${tab}ok${tab}-${tab}valid BMP image and pixel array" "imgcheck did not accept valid BMP"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample.tiff")" "$WORK_DIR/sample.tiff${tab}tiff${tab}ok${tab}-${tab}valid TIFF header and first IFD" "imgcheck did not validate TIFF first IFD"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample-bigtiff.tiff")" "$WORK_DIR/sample-bigtiff.tiff${tab}tiff${tab}ok${tab}-${tab}valid BigTIFF header and first IFD" "imgcheck did not validate BigTIFF first IFD"
assert_text_equals "$(${TEST_BIN_DIR}/imgcheck --plain "$WORK_DIR/sample.webp")" "$WORK_DIR/sample.webp${tab}webp${tab}ok${tab}-${tab}valid WebP RIFF image" "imgcheck did not validate WebP RIFF image"

assert_file_contains "$WORK_DIR/late-time.png" 'PNG' "strict-mode fixture sanity check failed"
"${TEST_BIN_DIR}/imgcheck" --plain "$WORK_DIR/late-time.png" > "$WORK_DIR/late-time-default.out"
...
if "${TEST_BIN_DIR}/imgcheck" --strict "$WORK_DIR/late-time.png" > "$WORK_DIR/late-time-strict.out" 2>&1; then
...
"${TEST_BIN_DIR}/imgcheck" --json "$WORK_DIR/valid.png" > "$WORK_DIR/valid-json.out"
...
"${TEST_BIN_DIR}/imgcheck" --json "$WORK_DIR/c2pa.jpg" > "$WORK_DIR/c2pa-json.out"
...
if [ -f /home/mathias/c2pa/2.2/image/good/jpeg/a.jpg ]; then
    "${TEST_BIN_DIR}/imgcheck" --json /home/mathias/c2pa/2.2/image/good/jpeg/a.jpg > "$WORK_DIR/c2pa-corpus-json.out"
...
"${TEST_BIN_DIR}/imgcheck" --recursive --json "$WORK_DIR/nested" > "$WORK_DIR/recursive-json.out"
...
if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/bad-crc.png" > "$WORK_DIR/bad-crc.out" 2>&1; then
...
"${TEST_BIN_DIR}/imgcheck" --json "$WORK_DIR/bad-crc.png" > "$WORK_DIR/bad-crc-json.out" 2>&1 || true
assert_file_contains "$WORK_DIR/bad-crc-json.out" '"failure_offset":8' "imgcheck --json did not report bad PNG CRC offset"

if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/no-idat.png" > "$WORK_DIR/no-idat.out" 2>&1; then
...
if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/bad-segment.jpg" > "$WORK_DIR/bad-jpeg.out" 2>&1; then
...
if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/truncated.gif" > "$WORK_DIR/truncated-gif.out" 2>&1; then
...
if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/truncated.bmp" > "$WORK_DIR/truncated-bmp.out" 2>&1; then
...
if "${TEST_BIN_DIR}/imgcheck" "$WORK_DIR/not-image.txt" > "$WORK_DIR/not-image.out" 2>&1; then
...
cat "$WORK_DIR/valid.png" | "${TEST_BIN_DIR}/imgcheck" -q
if cat "$WORK_DIR/bad-crc.png" | "${TEST_BIN_DIR}/imgcheck" -q; then
