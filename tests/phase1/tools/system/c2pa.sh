...
"${TEST_BIN_DIR}/imgcheck" --plain "$WORK_DIR/claimed.png" > "$WORK_DIR/imgcheck.out"
...
"${TEST_BIN_DIR}/imgmeta" show -v "$WORK_DIR/claimed.png" > "$WORK_DIR/imgmeta.out"
...
if "${TEST_BIN_DIR}/grep" -q 'c2pa.hash.data sha256=cda160f6dbf0e55b822788dcec33fc8cb5b7f2a1b18ad951d655a8a39d382d02' "$WORK_DIR/imgmeta.out"; then
    fail "c2pa add should hash the c2pa.hash.data JUMBF box, not the raw assertion CBOR"
fi
if "${TEST_BIN_DIR}/grep" -q 'c2pa.actions.v2 sha256=d68fab5cc5ededf6d72f267104a3655e0346ac3b5fcee137cecf71625968a526' "$WORK_DIR/imgmeta.out"; then
...
"${TEST_BIN_DIR}/od" -A n -t x1 "$WORK_DIR/claimed.png" | "${TEST_BIN_DIR}/tr" -d ' \n' > "$WORK_DIR/claimed.hex"
if "${TEST_BIN_DIR}/grep" -q '6a617373657274696f6e73' "$WORK_DIR/claimed.hex"; then
    fail "c2pa add should not write the legacy bare assertions claim key in c2pa.claim.v2"
fi
if "${TEST_BIN_DIR}/grep" -q '6f636c61696d5f67656e657261746f72' "$WORK_DIR/claimed.hex"; then
