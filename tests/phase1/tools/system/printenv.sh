...
value_out=$(FOO=bar "${TEST_BIN_DIR}/printenv" FOO | tr -d '\r\n')
assert_text_equals "$value_out" 'bar' "printenv NAME did not return the variable value"

name_out=$(FOO=bar "${TEST_BIN_DIR}/printenv" -n FOO | tr -d '\r\n')
assert_text_equals "$name_out" 'FOO' "printenv -n did not print only the variable name"

zero_out=$(FOO=bar "${TEST_BIN_DIR}/printenv" -0 FOO | tr '\0\r' '\n' | tr -d '\n')
assert_text_equals "$zero_out" 'bar' "printenv -0 did not use NUL-delimited output"

assert_command_succeeds env FOO=bar "${TEST_BIN_DIR}/printenv" -q FOO

missing_status=0
FOO=bar "${TEST_BIN_DIR}/printenv" DOES_NOT_EXIST >/dev/null 2>&1 || missing_status=$?
