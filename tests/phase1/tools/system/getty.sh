...
"${TEST_BIN_DIR}/getty" --help > "$WORK_DIR/getty_help.out"
assert_file_contains "$WORK_DIR/getty_help.out" '^Usage: .*getty' "getty --help did not print usage"

assert_command_succeeds "${TEST_BIN_DIR}/getty" -n -q -i /dev/null "${TEST_BIN_DIR}/true"

getty_status=0
"${TEST_BIN_DIR}/getty" -n -q -i /dev/null "${TEST_BIN_DIR}/sh" -c 'exit 9' >/dev/null 2>&1 || getty_status=$?
assert_exit_code "$getty_status" '9' "getty did not propagate the child exit status"

if "${TEST_BIN_DIR}/getty" -n -q >/dev/null 2>&1; then
    fail "getty without a tty path should fail"
fi

term_output=$(printf 'console-user\n' | "${TEST_BIN_DIR}/getty" -n -q -i -t vt100 -p 'login: ' -c 'printf "%s:%s" "$TERM" "$GETTY_USER"' - | tr -d '\r')
case "$term_output" in
    *'login: '*) : ;;
...
assert_command_succeeds "${TEST_BIN_DIR}/getty" -n -q --issue-file "$WORK_DIR/issue.txt" "$WORK_DIR/getty_tty.log" "${TEST_BIN_DIR}/true"
...
getty_path_status=0
PATH="$WORK_DIR/getty_fakebin:$PATH" "${TEST_BIN_DIR}/getty" -n -q -i /dev/null true >"$WORK_DIR/getty_path.out" 2>"$WORK_DIR/getty_path.err" || getty_path_status=$?
