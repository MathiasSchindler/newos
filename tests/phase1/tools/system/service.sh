...
cleanup_service() {
    if [ -n "$service_pid" ]; then
        "${TEST_BIN_DIR}/service" stop "$WORK_DIR/service.conf" >/dev/null 2>&1 || true
        kill "$service_pid" 2>/dev/null || true
    fi
}
trap 'cleanup_service' EXIT HUP INT TERM

"${TEST_BIN_DIR}/service" start "$WORK_DIR/service.conf" > "$WORK_DIR/start.out"
...
"${TEST_BIN_DIR}/service" stop "$WORK_DIR/service.conf" > "$WORK_DIR/stop.out" || true
...
EOF
unsafe_status=0
"${TEST_BIN_DIR}/service" start "$WORK_DIR/unsafe.conf" > "$WORK_DIR/unsafe.out" 2>&1 || unsafe_status=$?
