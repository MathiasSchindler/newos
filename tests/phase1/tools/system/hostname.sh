...
hostname_out=$("${TEST_BIN_DIR}/hostname" | tr -d '\r\n')
assert_nonempty_text "$hostname_out" "hostname output was empty"

uname_nodename_out=$("${TEST_BIN_DIR}/uname" -n | tr -d '\r\n')
assert_text_equals "$hostname_out" "$uname_nodename_out" "hostname and uname -n disagreed"

invalid_status=0
"${TEST_BIN_DIR}/hostname" 'bad name' >/dev/null 2>"$WORK_DIR/invalid.err" || invalid_status=$?
...
invalid_status=0
"${TEST_BIN_DIR}/hostname" "$long_label" >/dev/null 2>"$WORK_DIR/long-label.err" || invalid_status=$?
