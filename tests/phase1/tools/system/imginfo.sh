...
while [ "$i" -lt 5 ]; do
    ...
    "${TEST_BIN_DIR}/yes" x | "${TEST_BIN_DIR}/head" -c 60000 >> "$WORK_DIR/large-prefix.jpg"
    i=$((i + 1))
done
...
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.png")" "$WORK_DIR/sample.png${tab}png${tab}2${tab}3${tab}8${tab}4${tab}image/png" "imginfo did not parse PNG metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.gif")" "$WORK_DIR/sample.gif${tab}gif${tab}4${tab}5${tab}1${tab}3${tab}image/gif" "imginfo did not parse GIF metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.jpg")" "$WORK_DIR/sample.jpg${tab}jpeg${tab}6${tab}7${tab}8${tab}3${tab}image/jpeg" "imginfo did not parse JPEG metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/large-prefix.jpg")" "$WORK_DIR/large-prefix.jpg${tab}jpeg${tab}6${tab}7${tab}8${tab}3${tab}image/jpeg" "imginfo did not scan large JPEG metadata prefixes"
assert_text_equals "$(cat "$WORK_DIR/large-prefix.jpg" | "${TEST_BIN_DIR}/imginfo" --plain)" "stdin${tab}jpeg${tab}6${tab}7${tab}8${tab}3${tab}image/jpeg" "imginfo did not scan large JPEG metadata prefixes from stdin"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.tiff")" "$WORK_DIR/sample.tiff${tab}tiff${tab}9${tab}10${tab}8${tab}3${tab}image/tiff" "imginfo did not parse TIFF metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample-bigtiff.tiff")" "$WORK_DIR/sample-bigtiff.tiff${tab}tiff${tab}11${tab}12${tab}8${tab}3${tab}image/tiff" "imginfo did not parse BigTIFF metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.webp")" "$WORK_DIR/sample.webp${tab}webp${tab}12${tab}13${tab}-${tab}4${tab}image/webp" "imginfo did not parse WebP metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --plain "$WORK_DIR/sample.bmp")" "$WORK_DIR/sample.bmp${tab}bmp${tab}13${tab}14${tab}24${tab}3${tab}image/bmp" "imginfo did not parse BMP metadata"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --mime "$WORK_DIR/sample.png")" "$WORK_DIR/sample.png: image/png" "imginfo --mime did not print PNG MIME type"
assert_text_equals "$(${TEST_BIN_DIR}/imginfo --canonical-ext "$WORK_DIR/sample.png")" "png" "imginfo --canonical-ext did not print PNG extension"
assert_file_contains "$WORK_DIR/sample.png" 'PNG' "test fixture sanity check failed"

"${TEST_BIN_DIR}/imginfo" --json "$WORK_DIR/sample.png" > "$WORK_DIR/png-json.txt"
...
"${TEST_BIN_DIR}/imginfo" --details "$WORK_DIR/sample.png" > "$WORK_DIR/png-details.txt"
...
"${TEST_BIN_DIR}/imginfo" --details "$WORK_DIR/sample.jpg" > "$WORK_DIR/jpeg-details.txt"
...
"${TEST_BIN_DIR}/imginfo" --details "$WORK_DIR/c2pa.jpg" > "$WORK_DIR/c2pa-jpeg-details.txt"
...
"${TEST_BIN_DIR}/imginfo" --json "$WORK_DIR/c2pa.jpg" > "$WORK_DIR/c2pa-jpeg-json.txt"
...
if [ -f /home/mathias/c2pa/2.2/image/good/jpeg/a.jpg ]; then
    "${TEST_BIN_DIR}/imginfo" --details /home/mathias/c2pa/2.2/image/good/jpeg/a.jpg > "$WORK_DIR/c2pa-corpus-jpeg-details.txt"
...
"${TEST_BIN_DIR}/imginfo" --details "$WORK_DIR/sample.webp" > "$WORK_DIR/webp-details.txt"
...
"${TEST_BIN_DIR}/imginfo" --details "$WORK_DIR/animated.webp" > "$WORK_DIR/animated-webp-details.txt"
...
"${TEST_BIN_DIR}/imginfo" "$WORK_DIR/misleading.jpg" > "$WORK_DIR/mismatch-out.txt" 2> "$WORK_DIR/mismatch-err.txt"
...
"${TEST_BIN_DIR}/imginfo" --recursive --json "$WORK_DIR/nested" > "$WORK_DIR/recursive-json.txt"
assert_file_contains "$WORK_DIR/recursive-json.txt" 'inner.png' "imginfo --recursive did not visit nested image"

if "${TEST_BIN_DIR}/imginfo" "$WORK_DIR/not-image.txt" >/dev/null 2>&1; then
    fail "imginfo should reject unsupported image data"
fi

assert_text_equals "$(cat "$WORK_DIR/sample.gif" | "${TEST_BIN_DIR}/imginfo" --plain)" "stdin${tab}gif${tab}4${tab}5${tab}1${tab}3${tab}image/gif" "imginfo did not parse stdin"
