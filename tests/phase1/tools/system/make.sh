...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -f Flags.mk -j4
)
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -f Export.mk
)
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -n -f Force.mk > force.out
)
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -f OneShell.mk
)
...
(
    cd "$WORK_DIR"
    start=$(date +%s)
    assert_command_succeeds "${TEST_BIN_DIR}/make" -f Parallel.mk -j2 > parallel.out
    end=$(date +%s)
    elapsed=$((end - start))
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -s -f Silent.mk > silent.out
)
silent_out=$(tr -d '\r\n' < "$WORK_DIR/silent.out")
...
(
    cd "$WORK_DIR"
    assert_command_succeeds "${TEST_BIN_DIR}/make" -f Always.mk
    assert_command_succeeds "${TEST_BIN_DIR}/make" -B -f Always.mk
)
...
(
    cd "$WORK_DIR"
    if "${TEST_BIN_DIR}/make" -f Recursive.mk > recursive.out 2> recursive.err; then
