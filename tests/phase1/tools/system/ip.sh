...
assert_command_succeeds "${TEST_BIN_DIR}/ip" --help > "$WORK_DIR/help.out"
assert_file_contains "$WORK_DIR/help.out" '^Usage: ' "ip --help did not print a usage banner"

assert_command_succeeds "${TEST_BIN_DIR}/ip" addr > "$WORK_DIR/addr.out"
...
assert_command_succeeds "${TEST_BIN_DIR}/ip" link show dev "$dev_name" > "$WORK_DIR/link.out"
...
assert_command_succeeds "${TEST_BIN_DIR}/ip" addr show "$dev_name" > "$WORK_DIR/addr_dev.out"
assert_file_contains "$WORK_DIR/addr_dev.out" ": $dev_name: <" "ip addr show IFACE did not accept the short compatibility form"

assert_command_succeeds "${TEST_BIN_DIR}/ip" -4 addr show > "$WORK_DIR/ipv4.out"
assert_file_contains "$WORK_DIR/ipv4.out" 'inet ' "ip -4 addr show did not keep IPv4 output"

assert_command_succeeds "${TEST_BIN_DIR}/ip" -br addr show > "$WORK_DIR/brief.out"
...
route_status=0
"${TEST_BIN_DIR}/ip" route > "$WORK_DIR/route.out" 2> "$WORK_DIR/route.err" || route_status=$?
if [ "$route_status" -ne 0 ]; then
    assert_file_contains "$WORK_DIR/route.err" 'not available on this platform' "ip route should report platform-unavailable route listing clearly"
    note "ip route not available on this platform; skipping route-detail checks"
    exit 0
fi
...
assert_command_succeeds "${TEST_BIN_DIR}/ip" route show dev "$dev_name" > "$WORK_DIR/route_dev.out"
...
if "${TEST_BIN_DIR}/ip" link show dev lo > "$WORK_DIR/lo_link.out" 2>/dev/null; then
    assert_command_succeeds "${TEST_BIN_DIR}/ip" route show dev lo > "$WORK_DIR/route_lo.out"
...
assert_command_succeeds "${TEST_BIN_DIR}/ip" -6 route show > "$WORK_DIR/route6.out"
...
if grep -q '^default' "$WORK_DIR/route.out"; then
    assert_command_succeeds "${TEST_BIN_DIR}/ip" route show default > "$WORK_DIR/default.out"
