...
assert_command_succeeds "${TEST_BIN_DIR}/init" -n -q "${TEST_BIN_DIR}/true"

init_status=0
"${TEST_BIN_DIR}/init" -n -q -c 'exit 7' >/dev/null 2>&1 || init_status=$?
assert_exit_code "$init_status" '7' "init did not propagate the child exit status in no-respawn mode"

: > "$WORK_DIR/init_console.log"
init_status=0
"${TEST_BIN_DIR}/init" -q -m 2 -t "$WORK_DIR/init_console.log" -e BOOT_STAGE=rescue -c 'printf "%s\n" "$BOOT_STAGE"; exit 1' >/dev/null 2>&1 || init_status=$?
...
init_path_status=0
PATH="$WORK_DIR/init_fakebin:$PATH" "${TEST_BIN_DIR}/init" -n -q true >"$WORK_DIR/init_path.out" 2>"$WORK_DIR/init_path.err" || init_path_status=$?
