...
"${TEST_BIN_DIR}/clear" > "$WORK_DIR/clear.out"
...
"${TEST_BIN_DIR}/clear" --help > "$WORK_DIR/help.out"
assert_file_contains "$WORK_DIR/help.out" '^Usage: ' "clear --help did not print usage"

clear_status=0
"${TEST_BIN_DIR}/clear" unexpected >/dev/null 2>"$WORK_DIR/error.out" || clear_status=$?
