...
date_reference_out=$("${TEST_BIN_DIR}/date" -l -r "$WORK_DIR/reference.txt" +%Y-%m-%d | tr -d '\r\n')
assert_text_equals "$date_reference_out" '2000-01-02' "date -r did not read the file timestamp"

date_invalid_status=0
"${TEST_BIN_DIR}/date" -d @999999999999999999999999 >/dev/null 2>"$WORK_DIR/date_invalid.err" || date_invalid_status=$?
...
date_set_status=0
"${TEST_BIN_DIR}/date" -s @0 >/dev/null 2>"$WORK_DIR/date_set.err" || date_set_status=$?
