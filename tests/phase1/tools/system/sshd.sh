...
if command -v ssh >/dev/null 2>&1 && command -v timeout >/dev/null 2>&1; then
    ...
    while [ "$attempt" -le 2 ]; do
        ...
        if command -v sshpass >/dev/null 2>&1; then
            timeout 20 sshpass -p secret ssh -n \
                ...
                -p "$port" demo@127.0.0.1 'printf sshd-smoke' > "$WORK_DIR/sshd_smoke_client.out" 2>&1 < /dev/null || ssh_smoke_status=$?
        else
            printf '#!/bin/sh\nprintf "%%s\\n" secret\n' > "$WORK_DIR/askpass.sh"
            chmod +x "$WORK_DIR/askpass.sh"
            timeout 20 env SSH_ASKPASS="$WORK_DIR/askpass.sh" SSH_ASKPASS_REQUIRE=force DISPLAY=newos ssh -n \
                -o StrictHostKeyChecking=no \
                -o UserKnownHostsFile="$WORK_DIR/known_hosts" \
                -o PreferredAuthentications=password \
                -o PubkeyAuthentication=no \
    ...
    note "skipping sshd OpenSSH smoke; ssh and timeout are required"
fi
