...
bc_pow_tower_size=$("$ROOT_DIR/build/bc" '2^2^2^2^2' | wc -c | tr -d '[:space:]')
...
bc_extended_scale_size=$("$ROOT_DIR/build/bc" 'scale=128; 1/3' | wc -c | tr -d '[:space:]')
...
bc_math_library=$("$ROOT_DIR/build/bc" -l 'scale=10; e(1); l(1); l(10); a(1); s(pi/2); c(pi); j(0,0); j(1,0); exp(0); sin(pi)' | tr -d '\r')
