...
- parameter names declared with `--param NAME` for symbolic differentiation with respect to the selected variable only, or bound numerically with `--param NAME=VALUE` for numeric evaluation paths
...
- exact polynomial area between two curves, with numeric omitted-bound discovery from `--scan` for non-polynomial curves; rotation volume around the x-axis; and mean value over an interval
...
- `--param NAME` declares `NAME` as a symbolic parameter rather than the solving variable; `--param NAME=VALUE` additionally binds a numeric value for evaluation, numeric bounds, and numeric integration paths
...
- `--area A:B F G` computes area between two curves on `[A, B]`; if `A:B` is omitted, `--area F G` uses the leftmost and rightmost exact polynomial intersections when they can be proven, or roots found by the current `--scan` range for numeric omitted-bound area
...
- `--param NAME=VALUE` binds numeric parameters for evaluator-based modes, numeric bounds, and numeric integration fallback; exact rational polynomial solving and exact polynomial integration still require literal rational expressions in the parsed expression or bounds
...
- `--area F G` without explicit bounds requires at least two exact polynomial intersections, or a `--scan` range that finds at least two numeric intersections; numeric omitted-bound areas are approximate and bounded by the scan range
