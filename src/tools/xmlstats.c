...
static int ensure_name_capacity(NameCount **names_io, size_t count, size_t *capacity, NameCount *inline_names, size_t needed) {
    ...
    names = (NameCount *)rt_malloc(next_capacity * sizeof(*names));
    if (names == 0) return -1;
...
static int add_name(NameCount **names_io, size_t *count, size_t *capacity, NameCount *inline_names, const XmlName *name) {
    NameCount *names = *names_io;
    size_t i;
    for (i = 0U; i < *count; ++i) {
        if (names[i].length == name->length && rt_strncmp(names[i].name, name->start, name->length) == 0) {
            names[i].count += 1ULL;
            return 0;
        }
    }
...
static int stats_one(const char *path, XmlStats *stats) {
    ...
    while ((result = xml_next_token(&parser, &token)) > 0) {
        if (token.type == XML_TOKEN_START || token.type == XML_TOKEN_EMPTY) {
            ...
            if (add_name(&stats->element_names, &stats->element_name_count, &stats->element_name_capacity, stats->inline_element_names, &token.name) != 0) {
                xml_free_document(input);
                return 1;
            }
            for (i = 0U; i < token.attribute_count; ++i) {
                if (add_name(&stats->attribute_names, &stats->attribute_name_count, &stats->attribute_name_capacity, stats->inline_attribute_names, &token.attributes[i].name) != 0) {
                    xml_free_document(input);
                    return 1;
                }
            }
