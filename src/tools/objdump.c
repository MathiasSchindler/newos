...
static int parse_pe_header(int fd, PeHeaderInfo *info) {
    ...
    info->machine = tool_read_u16_le(coff + 4);
    info->section_count = tool_read_u16_le(coff + 6);
    info->timestamp = tool_read_u32_le(coff + 8);
    info->optional_size = tool_read_u16_le(coff + 20);
    info->characteristics = tool_read_u16_le(coff + 22);
    ...
    if (info->optional_size >= 70U) {
        ...
        info->optional_magic = tool_read_u16_le(optional + 0);
        info->entry = (unsigned long long)tool_read_u32_le(optional + 16);
        if (info->optional_magic == 0x020bU) {
            info->image_base = tool_read_u64_le(optional + 24);
        } else if (info->optional_magic == 0x010bU) {
            info->image_base = (unsigned long long)tool_read_u32_le(optional + 28);
        }
        info->subsystem = tool_read_u16_le(optional + 68);
    } else if (info->optional_size >= 2U) {
        unsigned char magic[2];
        if (read_region(fd, optional_offset, magic, sizeof(magic)) != 0) {
            return -1;
        }
        info->optional_magic = tool_read_u16_le(magic);
    }
...
static int load_pe_sections(int fd, const PeHeaderInfo *header, PeSectionInfo *sections) {
    ...
    for (i = 0U; i < header->section_count; ++i) {
        ...
        tool_copy_printable_bytes(sections[i].name, sizeof(sections[i].name), raw + 0, 8U);
        sections[i].virtual_size = tool_read_u32_le(raw + 8);
        sections[i].virtual_address = tool_read_u32_le(raw + 12);
...
static void print_file_header(const char *path, const ElfHeaderInfo *header) {
    ...
    tool_write_hex_value(1, header->entry);
    rt_write_char(1, '\n');
}
...
static void print_macho_file_header(const char *path, const MachHeaderInfo *header) {
    ...
    tool_write_hex_value(1, (unsigned long long)header->flags);
    rt_write_char(1, '\n');
}
...
static void print_pe_section_table(const PeHeaderInfo *header, const PeSectionInfo *sections) {
    ...
    for (i = 0U; i < header->section_count; ++i) {
        ...
        tool_write_hex_value(1, (unsigned long long)sections[i].virtual_address);
        rt_write_cstr(1, " vsize=");
        tool_write_hex_value(1, (unsigned long long)sections[i].virtual_size);
        rt_write_cstr(1, " off=");
        tool_write_hex_value(1, (unsigned long long)sections[i].raw_offset);
        rt_write_cstr(1, " size=");
        tool_write_hex_value(1, (unsigned long long)sections[i].raw_size);
        rt_write_cstr(1, " flags=");
        tool_write_hex_value(1, (unsigned long long)sections[i].characteristics);
        rt_write_char(1, '\n');
    }
}
...
static void dump_bytes_range(int fd, const char *name, unsigned long long offset, unsigned long long size, unsigned long long addr) {
    ...
    while (remaining > 0ULL) {
        ...
        tool_write_hex_value(1, display_addr);
        rt_write_cstr(1, " ");
        for (i = 0U; i < 16U; ++i) {
            if (i < chunk) {
...
static int json_macho_symbols(int fd, const char *path, const MachSymtabInfo *symtab) {
    ...
    for (index = 0U; index < symtab->nsyms; ++index) {
        ...
        strx = tool_read_u32_le(entry + 0);
        if (json_symbol_event(path, "mach-o-64", tool_read_u64_le(entry + 8), name_from_table(strings, string_size, strx), (unsigned int)entry[4], (unsigned int)entry[5]) != 0) return -1;
    }
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    for (i = argi; i < argc; ++i) {
        ...
        if (parse_elf_header(fd, &header) != 0 || object_elf_load_sections(fd, objdump_object_base, 0ULL, header.shoff, header.shnum, header.shentsize, sections, OBJDUMP_MAX_SECTIONS) != 0 ||
            ...
            if (object_macho_parse_header(fd, objdump_object_base, 0ULL, &macho) == 0 &&
                object_macho_load_layout(fd, objdump_object_base, 0ULL, &macho, macho_sections, OBJDUMP_MAX_SECTIONS, OBJDUMP_MACHO_MAX_COMMANDS, &macho_section_count, &macho_symtab) == 0) {
                if (show_file) {
