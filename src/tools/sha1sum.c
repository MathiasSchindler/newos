...
static int print_digest_line(const unsigned char *digest, size_t digest_size, const char *label, int binary_mode, int zero_terminated) {
...
static int verify_manifest(int fd, int zero_terminated, int quiet, int status_only) {
    ...
    for (;;) {
        ...
        if (read_status < 0) {
            tool_write_error("sha1sum", "failed to read checksum list", 0);
            return 1;
        }
        if (read_status == 0) {
            break;
        ...
        {
            unsigned char expected[HASH_SHA1_SIZE];
            unsigned char actual[HASH_SHA1_SIZE];
            const char *path = 0;
            int target_fd;
            int should_close = 0;
            int ok = 0;
            ...
            if (parse_check_record(record, expected, &path) != 0) {
                if (!status_only) {
                    tool_write_error("sha1sum", "invalid checksum line", 0);
                }
                exit_code = 1;
                continue;
            }
            ...
            if (tool_open_input(path, &target_fd, &should_close) == 0 &&
                hash_sha1_stream(target_fd, actual) == 0 &&
                compare_digest(expected, actual, HASH_SHA1_SIZE)) {
                ok = 1;
            }
...
int main(int argc, char **argv) {
    ...
    while (argi < argc) {
        ...
        if (rt_strcmp(arg, "--binary") == 0) {
            binary_mode = 1;
            argi += 1;
            continue;
        }
        if (rt_strcmp(arg, "--text") == 0) {
            binary_mode = 0;
            argi += 1;
            continue;
        }
        ...
        for (j = 1; arg[j] != '\0'; ++j) {
            if (arg[j] == 'b') {
                ...
                tool_write_usage(argv[0], "[-bct] [-q] [-s] [-z] [FILE ...]");
                return 1;
            }
        }
        argi += 1;
    ...
    if (check_mode) {
        ...
        for (i = argi; i < argc; ++i) {
            ...
            if (tool_open_input(argv[i], &fd, &should_close) != 0) {
                rt_write_cstr(2, "sha1sum: cannot open ");
    ...
    if (argi >= argc) {
        unsigned char digest[HASH_SHA1_SIZE];
        if (hash_sha1_stream(0, digest) != 0 || print_digest_line(digest, HASH_SHA1_SIZE, "-", binary_mode, zero_terminated) != 0) {
            return 1;
        }
        return 0;
    }
    ...
    for (i = argi; i < argc; ++i) {
        ...
        if (hash_sha1_stream(fd, digest) != 0 || print_digest_line(digest, HASH_SHA1_SIZE, argv[i], binary_mode, zero_terminated) != 0) {
            rt_write_cstr(2, "sha1sum: failed on ");
            rt_write_line(2, argv[i]);
            exit_code = 1;
        }
