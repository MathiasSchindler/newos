...
static const unsigned char expack_lzss_bcj_rip_stub_x86_64[] = {
    0x31, 0xed, 0x49, 0x89, 0xe4, 0x4c, 0x8b, 0x35, 0x2a, 0x02, 0x00, 0x00,
    0x31, 0xff, 0x4c, 0x89, 0xf6, 0xba, 0x03, 0x00, 0x00, 0x00, 0x41, 0xba,
    0x22, 0x00, 0x00, 0x00, 0x41, 0xb8, 0xff, 0xff, 0xff, 0xff, 0x45, 0x31,
    0xc9, 0xb8, 0x09, 0x00, 0x00, 0x00, 0x0f, 0x05, 0x48, 0x85, 0xc0, 0x0f,
    0x88, 0xf4, 0x01, 0x00, 0x00, 0x49, 0x89, 0xc5, 0x4f, 0x8d, 0x7c, 0x35,
    0x00, 0x48, 0x8d, 0x1d, 0x0a, 0x02, 0x00, 0x00, 0x48, 0x8b, 0x2d, 0xf3,
    0x01, 0x00, 0x00, 0x48, 0x01, 0xdd, 0x4c, 0x89, 0xef, 0x45, 0x31, 0xc0,
    0x45, 0x31, 0xc9, 0x4c, 0x39, 0xff, 0x0f, 0x83, 0x95, 0x00, 0x00, 0x00,
    ...
    0x3d, 0x6d, 0x00, 0x00, 0x00, 0x31, 0xf6, 0x0f, 0x05, 0x48, 0x85, 0xc0,
    ...
    0xe5, 0xb8, 0x42, 0x01, 0x00, 0x00, 0x89, 0xdf, 0x48, 0x8d, 0x35, 0x3a,
    ...
    0x05, 0x90, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x88, 0x77,
    0x66, 0x55, 0x44, 0x33, 0x22, 0x11, 0x65, 0x78, 0x70, 0x61, 0x63, 0x6b,
    0x00, 0x00
};

static const unsigned char expack_lz4_stub_x86_64[] = {
    0xfc, 0x31, 0xed, 0x49, 0x89, 0xe4, 0x4c, 0x8b, 0x35, 0x87, 0x01, 0x00,
    ...
    0x35, 0x00, 0x48, 0x8d, 0x1d, 0x67, 0x01, 0x00, 0x00, 0x48, 0x8b, 0x2d,
    0x50, 0x01, 0x00, 0x00, 0x48, 0x01, 0xdd, 0x4c, 0x89, 0xef, 0x48, 0x39,
    ...
    0x3f, 0x01, 0x00, 0x00, 0x48, 0x8d, 0x3d, 0x6d, 0x00, 0x00, 0x00, 0x31,
    ...
    0x89, 0xdf, 0x48, 0x8d, 0x35, 0x3a, 0x00, 0x00, 0x00, 0x49, 0x8d, 0x54,
    0x24, 0x08, 0x4d, 0x8b, 0x14, 0x24, 0x4f, 0x8d, 0x54, 0xd4, 0x10, 0x41,
    0xb8, 0x00, 0x10, 0x00, 0x00, 0x0f, 0x05, 0xb8, 0x3c, 0x00, 0x00, 0x00,
    0xbf, 0x7f, 0x00, 0x00, 0x00, 0x0f, 0x05, 0x90, 0x11, 0x22, 0x33, 0x44,
    0x55, 0x66, 0x77, 0x88, 0x88, 0x77, 0x66, 0x55, 0x44, 0x33, 0x22, 0x11,
    0x65, 0x78, 0x70, 0x61, 0x63, 0x6b, 0x00, 0x00
};
...
#define EXPACK_LZREP_LOOKAHEAD 64U
#define EXPACK_LZREP_BEAM_WIDTH 12U
#define EXPACK_LZREP_EXPLICIT_CHOICES 3U
...
static void expack_lzrep_insert_state(ExpackLzrepBeamState *bucket, const ExpackLzrepBeamState *state) {
    unsigned int slot;
    unsigned int worst_slot = 0U;

    for (slot = 0U; slot < EXPACK_LZREP_BEAM_WIDTH; ++slot) {
...
static void expack_lzrep_add_transition(ExpackLzrepBeamState states[EXPACK_LZREP_LOOKAHEAD + 1U][EXPACK_LZREP_BEAM_WIDTH],
                                        unsigned int limit,
    ...
    expack_lzrep_insert_state(states[next_position], &next_state);
}

static void expack_choose_lzrep_token(const unsigned char *data, size_t size, size_t position, unsigned int last_distance, unsigned int bit_index, ExpackLzrepToken *token_out) {
    ExpackLzrepBeamState states[EXPACK_LZREP_LOOKAHEAD + 1U][EXPACK_LZREP_BEAM_WIDTH];
    size_t remaining = size - position;
    unsigned int limit = remaining > EXPACK_LZREP_LOOKAHEAD ? EXPACK_LZREP_LOOKAHEAD : (unsigned int)remaining;
    unsigned int rel_position;
    unsigned int slot;
    ...
    for (rel_position = 0U; rel_position < limit; ++rel_position) {
        ...
        ExpackLzrepMatch matches[EXPACK_LZREP_EXPLICIT_CHOICES];

        expack_find_lzrep_matches(data, size, absolute_position, matches, EXPACK_LZREP_EXPLICIT_CHOICES);
        for (slot = 0U; slot < EXPACK_LZREP_BEAM_WIDTH; ++slot) {
            ...
            token.length = 1U;
            token.distance = 0U;
            expack_lzrep_add_transition(states, limit, rel_position, &state, &token);
            ...
            for (match_index = 0U; match_index < EXPACK_LZREP_EXPLICIT_CHOICES && matches[match_index].length >= COMPRESSION_LZSS_MIN_MATCH; ++match_index) {
...
static int expack_compress_lzrep(const unsigned char *data, size_t size, unsigned char **payload_out, size_t *payload_size_out) {
    ...
    while (input_offset < size) {
        ...
        for (bit = 0U; bit < 8U && input_offset < size; ++bit) {
            ...
            expack_choose_lzrep_token(data, size, input_offset, last_distance, bit, &token);
