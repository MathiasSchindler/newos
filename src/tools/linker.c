...
static void print_usage(const char *program_name) {
    tool_write_usage(program_name, "[-o output] [-m elf_x86_64] [--target=elf64-x86-64|mach-o-arm64|pe-arm64] [--tiny] [--pack] [--macho-compact] [--gc-sections] [--icf=safe|all] [--call-graph-order] [--symbol-ordering-file FILE] [--call-graph-profile FILE] [--stats] [--map FILE] [--print-gc-sections] [--lto-cc=<cc>] object-or-archive ...");
}
...
int main(int argc, char **argv) {
    ...
    for (i = 1; i < expanded_argc; ++i) {
        ...
        if (parsing_options && rt_strcmp(arg, "--pack") == 0) {
            options.pack = 1;
            continue;
        }
        if (parsing_options && rt_strcmp(arg, "--no-pack") == 0) {
            options.pack = 0;
            continue;
        }
        ...
        if (parsing_options && rt_strcmp(arg, "--icf=all") == 0) {
            options.icf_safe = 1;
            options.icf_all = 1;
            continue;
        }
