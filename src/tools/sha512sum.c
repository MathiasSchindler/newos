...
static int verify_manifest(int fd, int zero_terminated, int quiet, int status_only) {
    ...
    for (;;) {
        ...
        if (read_status < 0) {
            tool_write_error("sha512sum", "failed to read checksum list", 0);
            return 1;
        }
        if (read_status == 0) {
            break;
...

        {
            ...
            if (parse_check_record(record, expected, &path) != 0) {
                if (!status_only) {
                    tool_write_error("sha512sum", "invalid checksum line", 0);
                }
                exit_code = 1;
                continue;
            }
            ...
            if (tool_open_input(path, &target_fd, &should_close) == 0 &&
                hash_sha512_stream(target_fd, actual) == 0 &&
                compare_digest(expected, actual, HASH_SHA512_SIZE)) {
                ok = 1;
            }
...
int main(int argc, char **argv) {
    ...
    if (argi >= argc) {
        ...
        if (hash_sha512_stream(0, digest) != 0 || print_digest_line(digest, HASH_SHA512_SIZE, "-", zero_terminated) != 0) {
            return 1;
        }
        return 0;
    }
    ...
    for (i = argi; i < argc; ++i) {
        ...
        if (hash_sha512_stream(fd, digest) != 0 || print_digest_line(digest, HASH_SHA512_SIZE, argv[i], zero_terminated) != 0) {
