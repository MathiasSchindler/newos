...
static int verify_manifest(int fd, int zero_terminated, int quiet, int status_only) {
    ...
    for (;;) {
        ...
        if (read_status < 0) {
            tool_write_error("md5sum", "failed to read checksum list", 0);
            return 1;
        }
        if (read_status == 0) {
            break;
        ...
        {
            unsigned char expected[HASH_MD5_SIZE];
            unsigned char actual[HASH_MD5_SIZE];
            const char *path = 0;
            int target_fd;
            int should_close = 0;
            int ok = 0;
            ...
            if (parse_check_record(record, expected, &path) != 0) {
                if (!status_only) {
                    tool_write_error("md5sum", "invalid checksum line", 0);
                }
                exit_code = 1;
                continue;
            }
            ...
            if (tool_open_input(path, &target_fd, &should_close) == 0 &&
                hash_md5_stream(target_fd, actual) == 0 &&
                compare_digest(expected, actual, HASH_MD5_SIZE)) {
                ok = 1;
            }
...
int main(int argc, char **argv) {
    ...
    if (check_mode) {
        ...
        for (i = argi; i < argc; ++i) {
            ...
            if (tool_open_input(argv[i], &fd, &should_close) != 0) {
                rt_write_cstr(2, "md5sum: cannot open ");
    ...
    if (argi >= argc) {
        unsigned char digest[HASH_MD5_SIZE];
        if (hash_md5_stream(0, digest) != 0 || print_digest_line(digest, HASH_MD5_SIZE, "-", zero_terminated) != 0) {
            return 1;
        }
        return 0;
    }
    ...
    for (i = argi; i < argc; ++i) {
        ...
        if (hash_md5_stream(fd, digest) != 0 || print_digest_line(digest, HASH_MD5_SIZE, argv[i], zero_terminated) != 0) {
            rt_write_cstr(2, "md5sum: failed on ");
            rt_write_line(2, argv[i]);
            exit_code = 1;
        }
