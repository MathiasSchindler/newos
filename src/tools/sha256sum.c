...
int main(int argc, char **argv) {
    ...
    if (argc == 1) {
        ...
        if (hash_sha256_stream(0, digest) != 0 || print_digest_line(digest, HASH_SHA256_SIZE, "-") != 0) {
            return 1;
        }
        return 0;
    }
    ...
    for (i = 1; i < argc; ++i) {
        ...
        if (hash_sha256_stream(fd, digest) != 0 || print_digest_line(digest, HASH_SHA256_SIZE, argv[i]) != 0) {
