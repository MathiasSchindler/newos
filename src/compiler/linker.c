...
#if COMPILER_LINKER_ENABLE_CONST_MERGE
    if (options != 0 && options->merge_constants && merge_const_sections(linker_objects, object_count, error_out, error_size) != 0) {
        return -1;
    }
#endif
...
int compiler_link_static_options(const char *const *object_paths,
    ...
    if (target == COMPILER_LINKER_TARGET_PE32PLUS_AARCH64) {
        return compiler_link_pe32plus_aarch64_static_options(object_paths, object_count, output_path, options, error_out, error_size);
    }
