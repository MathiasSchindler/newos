...
int tls13_build_client_hello(
	...
	// legacy_session_id
	if (wb_put_u8(&b, (unsigned char)legacy_session_id_len) != 0) return -1;
	...
	if (wb_put_u8(&b, 0) != 0) return -1;
	// host_name length
	if (wb_put_u16(&b, (unsigned short)sni_len) != 0) return -1;
	if (wb_put_bytes(&b, (const unsigned char *)sni, sni_len) != 0) return -1;
