...
static void hmac_sha384_prepare_key_block(
    unsigned char key_block[CRYPTO_SHA512_BLOCK_SIZE],
    const unsigned char *key,
    size_t key_len
) {
    size_t i;

    for (i = 0; i < CRYPTO_SHA512_BLOCK_SIZE; ++i) key_block[i] = 0U;
    if (key == 0 || key_len == 0U) return;
    if (key_len > CRYPTO_SHA512_BLOCK_SIZE) {
        crypto_sha384_hash(key, key_len, key_block);
        return;
    }
...
void crypto_hmac_sha384(
    ...
    unsigned char key_block[CRYPTO_SHA512_BLOCK_SIZE];
    unsigned char inner[CRYPTO_SHA384_DIGEST_SIZE];
    size_t i;

    if (out == 0 || (key == 0 && key_len != 0U) || (data == 0 && data_len != 0U)) return;
    hmac_sha384_prepare_key_block(key_block, key, key_len);
    for (i = 0; i < CRYPTO_SHA512_BLOCK_SIZE; ++i) key_block[i] ^= 0x36U;
    crypto_sha384_init(&ctx);
    crypto_sha512_update(&ctx, key_block, sizeof(key_block));
    if (data_len != 0U) crypto_sha512_update(&ctx, data, data_len);
    crypto_sha384_final(&ctx, inner);

    for (i = 0; i < CRYPTO_SHA512_BLOCK_SIZE; ++i) key_block[i] ^= (unsigned char)(0x36U ^ 0x5cU);
    crypto_sha384_init(&ctx);
    crypto_sha512_update(&ctx, key_block, sizeof(key_block));
    crypto_sha512_update(&ctx, inner, sizeof(inner));
